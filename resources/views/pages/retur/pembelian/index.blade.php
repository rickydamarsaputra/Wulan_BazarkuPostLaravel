...
@section('title', 'Retur Pembelian Page')

@section('content')
<div class="section-header d-flex justify-content-between">
  <h1>@yield('title')</h1>
  <a href="" class="btn btn-primary text-capitalize">tambah retur pembelian</a>
</div>
<div class="card">
  <div class="card-body">
    <form action="" method="POST" id="bazarku__form__filter__retur__pembelian">
...
<div class="card" id="bazarku__retur__penjualan__info">
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-striped" id="data__table__retur__pembelian">
        <thead>
...
<script>
  ...
  $(document).ready(() => {
    $('#bazarku__form__filter__retur__pembelian').on('submit', (e) => {
      e.preventDefault();
      const dateRange = $('#bazarku__date__range').val();
      const divisi = $('#bazarku__choose__divisi').val();

      let requestURL = "{{route('datatables.retur.pembelian', [':divisi', ':dateRange'])}}";
      ...
      if ($.fn.dataTable.isDataTable('#data__table__retur__pembelian')) {
        $('#data__table__retur__pembelian').DataTable().destroy();
        $('#data__table__retur__pembelian').DataTable({
          processing: true,
          serverSide: true,
          ajax: requestURL,
          columns: [{
